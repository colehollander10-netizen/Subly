...
|---|---|---|
| `id` | UUID | existing |
| `name` | String | service name, existing |
| `entryType` | `EntryType` enum | `.freeTrial` / `.subscription` |
| `status` | `Status` enum | `.active` / `.cancelled` / `.expired` |
| `chargeDate` | Date | **renamed from `trialEndDate`** — "when money leaves your account" |
| `chargeAmount` | Decimal | existing |
| `billingCycle` | `BillingCycle?` enum | `.monthly` / `.yearly` / `.weekly` / `.custom` — nil for trials, required for subscriptions |
| `notificationOffset` | Int? | per-entry override in days; nil = use global default |
| `cancelledAt` | Date? | set when status flips to `.cancelled`; used for "Caught $X" window |
